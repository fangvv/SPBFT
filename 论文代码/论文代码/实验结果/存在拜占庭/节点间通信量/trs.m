...
%http function 
fid = fopen('trs.txt','rt'); %首先打开文件把数据读取出来
...
%h.Color = [0 1 0];
hold on;
axis([4 22 45 1700]);%%%%%%%%坐标
set(gca,'XTickMode','manual','XTick',[4 7 10 13 16 19 22]); 
xlabel('\fontname{SimSun}节点数量','FontSize',14); 
ylabel('\fontname{SimSun}节点间通信量/\fontname{SimSun}次','FontSize',14);
title('\fontname{SimSun}PBFT-SHPBFT节点间通信量对比','FontSize',14)
...
legend( {'\fontname{Times New Roman}PBFT','\fontname{Times New Roman}SHPBFT'},'FontSize',12);
grid on
